...
| H4 | Gateway-side responder sends PUNCH_BYTE to requester eps | ✅ | _commit-pending_ | respond_to_punch() in punch.rs + start_dispatcher() in punch_agent.rs. Dispatcher decodes notifies, fires per-notify responder tasks (concurrent, not serialized). encode_punch_notify promoted to pub for H6 integration test use. 6 new tests (3 responder + 3 dispatcher including malformed-payload tolerance and clean channel-close shutdown). 896 lib tests pass |
