...
| H3 | PunchSocket — quinn AsyncUdpSocket wrapper intercepts 0x0B | ✅ | 566cb46 | Highest-risk task; H0 spike de-risked it. 5 new tests cover: PUNCH_NOTIFY intercept, PUNCH_BYTE drop, QUIC passthrough, interleaved batch compaction, empty-datagram tolerance. FakeSocket helper drives poll_recv without real UDP (no flake risk). 890 lib tests pass. fmt+clippy clean (no warnings on punch_socket.rs) |
| H4 | Gateway-side responder sends PUNCH_BYTE to requester eps | ✅ | f2372e7 | respond_to_punch() in punch.rs + start_dispatcher() in punch_agent.rs. Dispatcher decodes notifies, fires per-notify responder tasks (concurrent, not serialized). encode_punch_notify promoted to pub for H6 integration test use. 6 new tests (3 responder + 3 dispatcher including malformed-payload tolerance and clean channel-close shutdown). 896 lib tests pass |
| H5 | Wire --punch / --ns-server flags into ztlp listen | ✅ | c65c83b | --punch flag added; ns_server underscore removed; new QuicEndpoint::bind_with_socket_and_runtime() takes a custom Arc<dyn quinn::Runtime>; PunchRuntime wired when --punch && --ns-server; warning printed when --punch alone (no NS coordinator). PunchAgent starts keepalive + dispatcher when active. Keepalive uses a separate ephemeral socket (FUTURE: share Quinn's fd — tracked in code comment). ztlp listen --help shows --punch with documentation. 896 lib tests pass, no regressions |
| H6 | End-to-end integration test (fake NS, in-process) | ✅ | 83c991c | proto/tests/punch_e2e_test.rs — 3 BDD tests, 0.61s runtime, zero-flake design (127.0.0.1:0 ephemeral binds, tokio::time::timeout safety nets). Happy path + gateway-never-registered timeout + empty-PUNCH_NOTIFY timeout. 899 total tests pass (896 lib + 3 e2e), no regressions |
| H7 | NS-side test that PUNCH_NOTIFY uses :learned endpoint | ✅ | 7e886ac | ns/test/ztlp_ns/pick_best_notify_addr_test.exs — 4 BDD tests. Promoted `defp pick_best_notify_addr/1` → `def` (pure data fn, no side-effect risk). RED→GREEN captured: 4 UndefinedFunctionError before promote, 4/4 pass after. 736 NS tests pass (was 732 + 4 new), no regressions |
| R1 | NS client query — query_ns_for_relays() + NS handler | ✅ | f0245fb | New 0x0D LIST_RELAYS wire protocol end-to-end. Rust: punch.rs adds NS_LIST_RELAYS const, MAX_LIST_RELAYS_COUNT=32 cap, RelayListing{addr,region} struct, encode/decode_list_relays_request, encode/decode_list_relays_response, async query_ns_for_relays() that filters response by first byte. lib.rs re-exports public API. 12 new Rust tests (roundtrip empty/with-zone, malformed/short-buf rejection, cap-at-32, IPv4/IPv6 response roundtrip, family-mismatch rejection, fake-NS happy path, fake-NS timeout) — 908/908 lib pass. Elixir: server.ex adds process_query(<<0x0D, ...>>, source) clause + encode_list_relays_response/1, returns empty list (R2 wires the registration source). 4 new NS tests (unknown-zone-empty, empty-zone-request, response-shape, malformed-liveness) — 740/740 pass. fmt + clippy + mix compile --warnings-as-errors clean |
| R2 | Spawn probe task — drives `record_probe_success/failure` + failover_degraded/dead | ✅ | 1f23162 | proto/src/relay_pool.rs adds `probe_relay(sock, addr, timeout)` — 12-byte UDP probe `[0x5A, 0x37, 0xFE, 0x00, nonce::8 BE]`. v0.30.11-compatible (no relay-side changes; relays silently drop 0xFE type — success = send Ok + no ICMP within timeout, failure = ICMP-style io error). cmd_connect legacy path wraps FailoverOrchestrator in Arc<tokio::sync::Mutex<_>>, spawns probe task at `relay_probe_interval` cadence (MissedTickBehavior::Skip), walks `pool.relays_needing_probe()`, calls probe_relay over the shared node.socket, drives `record_probe_success(addr, latency)` / `record_probe_failure(addr)`. JoinHandle .abort()'d on return. 3 BDD tests (silent-listener success, bad-addr ICMP error w/ platform fallback, ZERO timeout no-panic). 911 lib tests pass (+3) |
| R3 | Replace hardcoded send_addr with relay_pool.primary() | ✅ | a2452d2 | cmd_connect (proto/src/bin/ztlp-cli.rs) now resolves `attempt_addr = orchestrator.lock().await.pool().primary().unwrap_or(send_addr)` ONCE before HELLO send, reuses for retransmits (so relay state machine doesn't reset). After handshake completion: `report_handshake_success(addr, start.elapsed())` on Ok, `report_handshake_failure(addr)` on socket recv error / retransmit exhaustion / incomplete handshake. Helper module proto/src/r3_pool_consult.rs holds the testable `resolve_handshake_attempt_addr()` function with 4 BDD tests (no-orchestrator fallback, primary override, failed-primary→backup shift after 16 probe failures, empty-pool fallback). 915 lib tests pass (+4 from R3). Build + fmt + clippy clean |
| H10 | Auto-punch / auto-pool when --ns-server present, --no-punch / --no-relay-pool escape hatches | ✅ | a39a49e | `ztlp connect <name> --ns-server <addr>` now auto-flips punch+pool ON. `ztlp listen --ns-server <addr>` auto-flips punch ON. Pure helper `proto/src/h10_defaults.rs::resolve_punch_and_pool_flags()` + 9 BDD tests pin every cell of the behaviour matrix. Clap `conflicts_with` rejects --punch+--no-punch (and the relay-pool pair) at parse time. Steve confirmed 10s keepalive (down from 25s) on 2026-05-27 after Z2LS bench stalls; both DEFAULT_KEEPALIVE_INTERVAL constants updated and the test_punch_config_defaults assertion re-anchored. 925 lib tests pass (was 915 + 9 H10 + 1 retuned). 3/3 e2e tests still pass. fmt+build clean |
| H8 | Bench validation on AWS + Steve's SD-WAN | ✅ | (this commit) | 2026-05-27 bench: rebuilt NS v0.30.12 + deployed Z2LS Windows gateway v0.30.12. Confirmed (a) H10 default-flip auto-enables punch from `--ns-server` alone — gateway log "Punch enabled — keepalive to 16.147.41.195:23096 every 10s"; (b) `ztlp connect <name> --ns-server <ns>` (Steve's canonical invocation) resolves SVC, queries NS PEER_ENDPOINTS, gets gateway's real SD-WAN public endpoint 204.16.122.24:60808, fires PUNCH_BYTE × 20 attempts at 500ms cadence; (c) punch timed out against symmetric SD-WAN edge → automatic relay fallback worked, tunnel established; (d) NS-kill mid-session survival CONFIRMED (tunnel intact for 30s+); (e) R2 probe task running in production logs ("probe success: 500ms"). Partial: relay-kill failover only weakly validated due to probe_relay()'s "no ICMP = healthy" heuristic (documented limitation, needs 0xFE echo on relay side for v0.31). Real pcap evidence + operational pitfalls captured in docs/NAT-TRAVERSAL.md |
| H9 | Update docs/NAT-TRAVERSAL.md with implementation status | ✅ | (this commit) | Added Implementation-status section with 14-row task table, default-behaviour summary, wire-protocol type-byte table (0x0A/0x0B/0x0C/0x0D + probe), keepalive=10s rationale, expected pcap sequence (placeholder for H8) |
