...
| T6 | BS: `ZtlpDevice` schema + `orphaned` status migration + tests | ✅ | `1a89011` | 10 new tests, 1132 BS total; adds `origin` (default `bootstrap`) + `last_synced_at` + index on `origin`; status enum expanded to pending/enrolled/revoked/orphaned, origin validated |
| T7 | BS: `Ztlp::SyncNsToBootstrap` reconciler + tests | ✅ | `062bd94` | 11 new tests, 49 assertions, ~0.7s; longest-zone-suffix routing (case-insensitive), idempotent upserts, orphan sweep, origin="bootstrap" immune, TransportError → Result(:error) without raising |
| T8 | BS: `ztlp:ns:sync` rake task + cron entrypoint | ✅ | `804f85d` | 2 new tests, 1145 BS total; rake task with greppable status line + AuditLog row (Result :ok/:error → "success"/"failure" to satisfy AuditLog validation); details persisted as JSON via existing column; docker-entrypoint spawns a 5-min `while true; do rake ztlp:ns:sync; sleep 300; done &` loop gated on `ZTLP_NS_SYNC_ENABLED=true`, teed to /rails/log/ns_sync.log; task itself never `exit`s (cron-safe + test-safe), opt-in hard-fail via `ZTLP_NS_SYNC_FAIL_HARD=true` |
| T9 | BS: Dashboard surfaces synced devices (UI badge + filter) | ✅ | _commit-pending_ | 4 new controller tests, 1149 BS total; Source column with `NS` (green) / `BS` (gray) badge; `last_synced_at` rendered as "synced X ago" subtitle under device name; status filter links extended with Pending + Orphaned; `params[:status]` whitelisted via `VALID_STATUSES` constant so junk values fall through (no SQL surface area) |
