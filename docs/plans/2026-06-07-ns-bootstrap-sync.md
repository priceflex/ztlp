...
| T7 | BS: `Ztlp::SyncNsToBootstrap` reconciler + tests | ✅ | _commit-pending_ | 11 new tests, 49 assertions, ~0.7s; longest-zone-suffix routing (case-insensitive), idempotent upserts, orphan sweep, origin="bootstrap" immune, TransportError → Result(:error) without raising |
