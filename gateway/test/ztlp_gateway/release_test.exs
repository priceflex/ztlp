defmodule ZtlpGateway.ReleaseTest do
  use ExUnit.Case, async: true

  @moduledoc """
  Tests that OTP release configuration is valid for the gateway service.
  """

  describe "mix.exs release config" do
    test "project has releases key" do
      project = ZtlpGateway.MixProject.project()
      assert Keyword.has_key?(project, :releases)
    end

    test "release name is :ztlp_gateway" do
      releases = ZtlpGateway.MixProject.project()[:releases]
      assert Keyword.has_key?(releases, :ztlp_gateway)
    end

    test "release strips beams" do
      release_opts = ZtlpGateway.MixProject.project()[:releases][:ztlp_gateway]
  ...
  describe "version reporting (regression pin)" do
    ...
    test "mix.exs version is at least 0.32.2 (v0.32.2 multi-candidate-QUIC-path tag)" do
      ...
      # gateway misreport itself as a pre-v0.32.2 version.
      ...
      # read 0.31.0; this PR closes that drift. Using Version.compare (rather
      # than asserting a literal string) means this test does NOT need
      # maintenance on every routine version bump — it only fails on a
      # down-bump below the 0.32.2 floor.
      declared = ZtlpGateway.MixProject.project()[:version]
      assert Version.compare(declared, "0.34.7") in [:gt, :eq],
             "mix.exs version #{declared} is older than the v0.34.7 desktop-enroll-paste-handler tag"
    end
  end
  ...
  describe "appup template" do
    test "appup file exists" do
      appup_path = Path.join([__DIR__, "..", "..", "rel", "appups", "ztlp_gateway.appup"])
      assert File.exists?(appup_path), "rel/appups/ztlp_gateway.appup must exist"
    end

    test "appup file is valid Erlang term" do
      appup_path =
        Path.join([__DIR__, "..", "..", "rel", "appups", "ztlp_gateway.appup"])
        |> Path.expand()
