defmodule ZtlpNs.EnrollmentTest do
  ...
  # ── v0.30.10 golden vector — cross-language MAC parity ────────────
  #
  # This vector pins HMAC-BLAKE2s byte-compatibility between the NS
  # (Elixir) and Launch (Python). It is generated by running
  # `python3 -c "..."` (see `ztlp.net/tests/test_launch_app.py
  # ::EnrollmentHmacSigningTest`) against the same canonical input. If
  # this test fails, either Launch's `_serialize_enrollment_token_for_signing`
  # or NS's `hmac_blake2s` has drifted. DO NOT regenerate the vector
  # blindly — investigate the drift first.
  #
  # Input parameters:
  #   secret_hex   = "aa" * 32      (32 bytes of 0xaa)
  #   zone         = "golden.example.ztlp"
  #   ns_addr      = "192.0.2.10:23096"
  #   relay_addrs  = ["192.0.2.20:23095"]
  #   gateway_addr = "192.0.2.30:23097"     (flag=0x01)
  #   max_uses     = 1
  #   expires_at   = 1893456000              (2030-01-01 UTC)
  #   nonce_hex    = "00112233445566778899aabbccddeeff"
  #
  # Canonical bytes (104 total), big-endian length prefixes:
  #   01                                                 # version
  #   01                                                 # flags (gw present)
  #   0013 + "golden.example.ztlp"                       # zone (19 bytes)
  #   0010 + "192.0.2.10:23096"                          # ns_addr (16 bytes)
  #   01                                                 # relay_count
  #   0010 + "192.0.2.20:23095"                          # relay (16 bytes)
  #   0010 + "192.0.2.30:23097"                          # gateway (16 bytes)
  #   0001                                               # max_uses
  #   0000000070dbd880                                   # expires (BE u64)
  #   00112233445566778899aabbccddeeff                   # nonce
  test "v0.30.10 golden vector: NS HMAC-BLAKE2s matches Launch canonical bytes" do
    secret = :binary.copy(<<0xAA>>, 32)

    # Canonical serialization (must match Launch's
    # _serialize_enrollment_token_for_signing byte-for-byte).
    canonical =
      <<0x01, 0x01>> <>
        <<19::16>> <> "golden.example.ztlp" <>
        <<16::16>> <> "192.0.2.10:23096" <>
        <<1>> <>
        <<16::16>> <> "192.0.2.20:23095" <>
        <<16::16>> <> "192.0.2.30:23097" <>
        <<1::16>> <>
        <<1_893_456_000::64>> <>
        <<0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD,
          0xEE, 0xFF>>

    assert byte_size(canonical) == 104, "canonical bytes drift — Launch and NS disagree"

    expected_mac_hex =
      "f320ecd5c86cd29835b106e9b82d371b3c09650baf13bcb1e0f61b5b0af907a1"

    actual_mac = Enrollment.hmac_blake2s(secret, canonical)
    assert Base.encode16(actual_mac, case: :lower) == expected_mac_hex
  end
end
