    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            while let Ok(frame) = rx.recv() {
                ...
                // Send the latest (coalesced) ACK — send REDUNDANT_SENDS times
                ...
                match build_encrypted_packet(session_id, &send_key, seq, &latest_frame) {
                    Ok(pkt) => {
                        for attempt in 0..REDUNDANT_SENDS {
                            if let Err(e) = raw_sendto(socket_fd, &pkt, &peer_addr) {
                                error_count += 1;
                                if error_count <= 50 {
                                    tracing::warn!("ack_sender: ACK send failed (fd={}, attempt={}, errno={}): {}", socket_fd, attempt, e.raw_os_error().unwrap_or(-1), e);
                                }
                            } else {
                                sent_count += 1;
                            }
                            if attempt < REDUNDANT_SENDS - 1 {
                                std::thread::sleep(std::time::Duration::from_micros(REDUNDANT_DELAY_US));
                            }
                        }
                    }
                    Err(e) => {
