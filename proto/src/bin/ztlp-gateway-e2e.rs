...
fn main() {
    ...
    socket.send_to(&start_msg, server_addr).expect("send start failed");
    ...
    if setup_data.len() < 32 {
        ...
        socket.send_to(&cmd1, server_addr).expect("send msg1 failed");
        ...
        match initiator.read_message(&msg2_data, &mut payload2) {
            Ok(_) => {
                ...
                socket.send_to(&cmd3, server_addr).expect("send msg3 failed");
                ...
                if initiator.is_handshake_finished() {
                    ...
                    if key_resp.starts_with(b"HANDSHAKE_OK") && key_resp.len() >= 12 + 64 {
                        ...
                        let i2r_key: [u8; 32] = key_resp[keys_start..keys_start + 32].try_into().unwrap();
                        let r2i_key: [u8; 32] = key_resp[keys_start + 32..keys_start + 64].try_into().unwrap();
                        ...
                        let ciphertext = cipher_i2r.encrypt(nonce, test_data.as_slice())
                        ...
                        socket.send_to(&data_msg, server_addr).expect("send data failed");
                        ...
                        if echo_resp.starts_with(b"GW_ECHO_DATA") {
                            ...
                            println!("✗ Unexpected echo response: {}", String::from_utf8_lossy(&echo_resp));
                            failed += 1;
                        }
                        ...
                        println!("✗ Handshake response: {}", String::from_utf8_lossy(&key_resp));
                        failed += 1;
                    }
                } else {
    ...
    socket.send_to(b"GW_SEND_TO_CLIENT", server_addr).expect("send request failed");
    ...
    socket.send_to(b"GW_POLICY_DENIED", server_addr).expect("send policy test failed");
    ...
    socket.send_to(b"GW_POLICY_ALLOWED", server_addr).expect("send policy test failed");
