...
fn test_full_handshake_in_process() {
    ...
    let result = perform_handshake(&initiator_id, &responder_id).expect("handshake should succeed");
    ...
    assert_eq!(
        result.initiator_session.peer_node_id, responder_id.node_id,
        "initiator should know responder's NodeID"
    );
    assert_eq!(
        result.responder_session.peer_node_id, initiator_id.node_id,
...
fn test_handshake_message_flow() {
    ...
    let mut initiator = HandshakeContext::new_initiator(&initiator_id).expect("create initiator");
    let mut responder = HandshakeContext::new_responder(&responder_id).expect("create responder");
...
fn test_handshake_with_payload() {
    ...
    let msg2 = responder
        .write_message(b"hello from responder")
        .expect("msg2");
    ...
    let msg3 = initiator
        .write_message(b"hello from initiator")
        .expect("msg3");
...
fn test_handshake_finalize_before_complete() {
    ...
    let mut initiator = HandshakeContext::new_initiator(&initiator_id).expect("create initiator");
    let mut _responder = HandshakeContext::new_responder(&responder_id).expect("create responder");
...
fn test_multiple_handshakes_produce_different_keys() {
    ...
    let result1 = perform_handshake(&initiator_id, &responder_id).expect("handshake 1");
    let result2 = perform_handshake(&initiator_id, &responder_id).expect("handshake 2");

    // Different sessions should have different keys (ephemeral key exchange)
    assert_ne!(
        result1.initiator_session.send_key, result2.initiator_session.send_key,
        "different handshakes should produce different keys"
    );
    assert_ne!(
        result1.initiator_session.session_id, result2.initiator_session.session_id,
