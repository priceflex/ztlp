...
async fn free_port(ip: Ipv4Addr) -> u16 {
    let l = TcpListener::bind(SocketAddr::new(ip.into(), 0))
        .await
        .unwrap();
...
fn add_service_updates_vip_when_re_added_with_new_address() {
    ...
    p.add_service("s".into(), Ipv4Addr::new(127, 0, 55, 1), 80)
        .unwrap();
    p.add_service("s".into(), Ipv4Addr::new(127, 0, 55, 2), 443)
        .unwrap();
    let s = &p.services()["s"];
    assert_eq!(
        s.vip,
        Ipv4Addr::new(127, 0, 55, 2),
        "VIP is updated on re-add"
    );
...
async fn session_ref_is_none_before_start_and_some_after() {
    ...
    let s = guard
        .as_ref()
        .expect("session stored even with zero services");
...
async fn tunnel_session_new_stores_fields() {
    ...
    let sc = Arc::new(tokio::sync::Mutex::new(
        ztlp_proto::send_controller::SendController::new(
            t.clone(),
            SessionId([4u8; 12]),
            peer(),
            rx,
...
async fn dispatcher_re_register_replaces_sender() {
    ...
    assert!(
        ...
        "old receiver's sender was dropped"
    );
...
async fn listener_accepts_connection_and_tunnel_data_reaches_tcp_client() {
    ...
    assert!(
        bytes_sent.load(Ordering::Relaxed) >= 3000,
        "bytes_sent tracks TCP reads"
    );
    // 3000 bytes / 1135 max mux payload = 3 frames.
    assert!(
        data_seq.load(Ordering::Relaxed) >= 3,
        "one data_seq per mux frame"
    );
...
async fn multiple_concurrent_connections_get_distinct_stream_ids() {
    ...
    for c in [&mut c1, &mut c2, &mut c3] {
        let mut b = [0u8; 8];
        let n = tokio::time::timeout(Duration::from_secs(2), c.read(&mut b))
            .await
            .unwrap()
            .unwrap();
...
async fn start_fails_cleanly_when_port_is_taken() {
    ...
    let holder = TcpListener::bind(SocketAddr::new(vip.into(), 0))
        .await
        .unwrap();
...
async fn stop_makes_listener_refuse_new_connections() {
    ...
    let r = tokio::time::timeout(
        ...
        TcpStream::connect(SocketAddr::new(vip.into(), port)),
    )
    .await;
    assert!(
...
async fn second_start_hot_swaps_session_and_resets_dispatcher() {
    ...
    let mut b = [0u8; 16];
    let n = tokio::time::timeout(Duration::from_secs(2), c2.read(&mut b))
        .await
        .unwrap()
        .unwrap();
...
async fn second_start_keeps_one_dispatcher_clears_old_streams_and_keeps_stream_ids_unique() {
    ...
    assert!(
        Arc::ptr_eq(&d1, &d2),
        "listeners and recv_loop must share ONE dispatcher across hot-swap"
    );
    assert_eq!(
        d2.stream_count(),
        0,
        "pre-swap streams are dropped (their session is gone)"
    );
    ...
    wait_streams(&d2, 1).await;
    assert!(
        matches!(d2.dispatch(1, vec![1]), Err(DispatchError::NoStream)),
        "stream_id 1 must not be reused"
    );
...
async fn update_session_swaps_without_touching_dispatcher() {
    ...
    assert!(
        Arc::ptr_eq(&d1, &proxy.dispatcher()),
        "update_session keeps dispatcher"
    );
...
async fn connection_before_any_session_is_rejected() {
    ...
    *proxy.session_ref().write().await = None;
    ...
    tokio::time::sleep(Duration::from_millis(200)).await;
    assert_eq!(
        dispatcher.stream_count(),
        0,
        "no stream registered without a session"
    );
    let mut b = [0u8; 4];
...
fn write_self_signed(cert_dir: &std::path::Path, name: &str) {
    ...
    let ck =
        rcgen::generate_simple_self_signed(vec![format!("{name}.techrockstars.ztlp")]).unwrap();
    std::fs::write(cert_dir.join(format!("{name}.pem")), ck.cert.pem()).unwrap();
    std::fs::write(
        cert_dir.join(format!("{name}.key")),
...
async fn tls_port_without_cert_falls_back_to_plain_tcp() {
    ...
    let port = 8443u16;
    if TcpListener::bind(SocketAddr::new(vip.into(), port))
        .await
        .is_err()
    {
...
async fn tls_port_with_cert_terminates_tls_and_rejects_plaintext_client() {
    ...
    assert_eq!(
        dispatcher.stream_count(),
        0,
        "handshake failure must not register a stream"
    );
    ...
    assert!(matches!(
        dispatcher.dispatch(1, vec![0]),
        Err(DispatchError::NoStream)
    ));
    dispatcher.dispatch(2, b"over-tls".to_vec()).unwrap();
    let mut b = [0u8; 16];
    let n = tokio::time::timeout(Duration::from_secs(2), tls.read(&mut b))
        .await
        .unwrap()
        .unwrap();
