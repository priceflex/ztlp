defmodule ZtlpRelay.NsClient do
  ...
  def register_self(zone, our_info),
    do: GenServer.call(__MODULE__, {:register_self, zone, our_info}, 10_000)
  ...
  @impl true
  def handle_call({:discover_relays, _}, _from, %{socket: nil} = s),
    do: {:reply, {:error, :no_socket}, s}

  def handle_call({:discover_relays, zone}, _from, s) do
    {:reply,
     wrap_discover_result(do_query_with_retry(s.socket, zone, @relay_type_byte, get_ns(s))), s}
  end

  def handle_call({:register_self, _, _}, _from, %{socket: nil} = s),
    do: {:reply, {:error, :no_socket}, s}
  ...
  def handle_call({:lookup_relay, _}, _from, %{socket: nil} = s),
    do: {:reply, {:error, :no_socket}, s}
      ...
      :miss ->
        ...
        case result do
          {:ok, rm} ->
            :ets.insert(
              @ns_cache,
              {name, {rm, System.system_time(:second) + Map.get(rm, :ttl, 3600)}}
            )
  ...
  defp cache_lookup(name) do
    case :ets.lookup(@ns_cache, name) do
      [{^name, {rm, exp}}] ->
        if System.system_time(:second) < exp,
          do: {:ok, rm},
          else:
            (fn ->
               :ets.delete(@ns_cache, name)
               :miss
             end).()
  ...
  defp resolve_host(h) when is_binary(h) do
    case :inet.parse_address(String.to_charlist(h)) do
      ...
      _ ->
        case :inet.getaddr(String.to_charlist(h), :inet) do
          {:ok, ip} -> {:ok, ip}
          {:error, r} -> {:error, r}
        end
    end
