defmodule ZtlpRelay.PathScore do
  ...
  load factor) and selects the best relay from a candidate set.

  Score formula: `RTT_ms * (1 + loss_rate * 10) * (1 + load_factor * 2)`

  Lower score = better relay. The formula heavily penalizes packet loss
  and moderately penalizes high load, while using RTT as the baseline.
  """
  ...
  ## Parameters
    - `metrics` — map with `:rtt_ms` (number), `:loss_rate` (0.0–1.0),
      `:load_factor` (0.0–1.0, ratio of active_sessions / max_sessions)

  ## Examples

      iex> ZtlpRelay.PathScore.compute(%{rtt_ms: 50, loss_rate: 0.0, load_factor: 0.0})
      50.0

      iex> ZtlpRelay.PathScore.compute(%{rtt_ms: 50, loss_rate: 0.1, load_factor: 0.5})
      100.0
  """
  @spec compute(metrics()) :: float()
  def compute(%{rtt_ms: rtt_ms, loss_rate: loss_rate, load_factor: load_factor}) do
    rtt_ms * (1.0 + loss_rate * 10.0) * (1.0 + load_factor * 2.0)
  end
