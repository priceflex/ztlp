defmodule ZtlpRelay.MeshManagerTest do
  ...
  setup do
    ...
    {:ok, reg_pid} = RelayRegistry.start_link(sweep_interval_ms: 60_000_000)
    ...
    {:ok, mm_pid} =
      MeshManager.start_link(
        ...
        # random port
        mesh_listen_port: 0,
        # very long, we don't want auto pings
        ping_interval_ms: 60_000_000,
        bootstrap_relays: []
      )
  ...
  describe "handle_inter_relay — RELAY_PONG" do
    test "updates scores for relay" do
      ...
      pong =
        InterRelay.encode_pong(other_node_id, %{
          active_sessions: 100,
          max_sessions: 10_000,
          uptime_seconds: 3600
        })
