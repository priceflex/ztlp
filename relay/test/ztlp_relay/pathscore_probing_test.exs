defmodule ZtlpRelay.PathScoreProbingTest do
  ...
  setup do
    try do
      :ets.new(:ztlp_relay_health, [:named_table, :set, :public, read_concurrency: true, write_concurrency: true])
    rescue
      ArgumentError -> :ok
    end
    try do
  ...
  describe "health state transitions" do
    ...
    test "healthy to degraded on moderate loss" do
      node_id = :crypto.strong_rand_bytes(16)
      result = RelayRegistry.update_health(node_id, loss_rate: 0.15, rtt_ms: 100.0, missed_sweeps: 0, pong_received: true)
    ...
    test "healthy to unreachable on high loss" do
      node_id = :crypto.strong_rand_bytes(16)
      result = RelayRegistry.update_health(node_id, loss_rate: 0.30, rtt_ms: 100.0, missed_sweeps: 0, pong_received: true)
      assert result == :unreachable
    end

    test "degraded to healthy requires 3 good pings" do
      node_id = :crypto.strong_rand_bytes(16)
      RelayRegistry.update_health(node_id, loss_rate: 0.10, rtt_ms: 100.0, missed_sweeps: 0, pong_received: true)
      assert RelayRegistry.get_health(node_id) == :degraded

      RelayRegistry.update_health(node_id, loss_rate: 0.01, rtt_ms: 50.0, missed_sweeps: 0, pong_received: true)
      assert RelayRegistry.get_health(node_id) == :degraded

      RelayRegistry.update_health(node_id, loss_rate: 0.01, rtt_ms: 50.0, missed_sweeps: 0, pong_received: true)
      assert RelayRegistry.get_health(node_id) == :degraded

      result = RelayRegistry.update_health(node_id, loss_rate: 0.01, rtt_ms: 50.0, missed_sweeps: 0, pong_received: true)
      assert result == :healthy
    end

    test "unreachable to degraded on pong" do
      node_id = :crypto.strong_rand_bytes(16)
      RelayRegistry.update_health(node_id, loss_rate: 0.30, rtt_ms: 100.0, missed_sweeps: 0, pong_received: true)
      assert RelayRegistry.get_health(node_id) == :unreachable

      result = RelayRegistry.update_health(node_id, loss_rate: 0.10, rtt_ms: 100.0, missed_sweeps: 0, pong_received: true)
      assert result == :degraded
    end
    ...
    test "unreachable stays unreachable without pong" do
      node_id = :crypto.strong_rand_bytes(16)
      RelayRegistry.update_health(node_id, loss_rate: 0.30, rtt_ms: 100.0, missed_sweeps: 0, pong_received: true)
      result = RelayRegistry.update_health(node_id, loss_rate: 0.10, rtt_ms: 100.0, missed_sweeps: 3, pong_received: false)
      assert result == :unreachable
    end
    ...
    test "good streak resets on bad ping" do
      ...
      RelayRegistry.update_health(node_id, loss_rate: 0.10, rtt_ms: 600.0, missed_sweeps: 0, pong_received: true)
      assert RelayRegistry.get_health(node_id) == :degraded
  ...
  describe "remove_health/1" do
    test "removes health tracking" do
      node_id = :crypto.strong_rand_bytes(16)
      RelayRegistry.update_health(node_id, loss_rate: 0.15, rtt_ms: 100.0, missed_sweeps: 0, pong_received: true)
      assert RelayRegistry.get_health(node_id) == :degraded
