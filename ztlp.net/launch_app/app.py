...
LAUNCH_NS_SERVER = os.environ.get("ZTLP_NS_SERVER") or "44.230.7.100:23096"
# The per-tenant gateway + register_ns sidecars run the Rust `ztlp` CLI
# (ztlp keygen / listen / ns register) under a `sh -c` one-liner, so this
# MUST be an image that ships the Rust binary at /usr/local/bin/ztlp AND a
# shell — i.e. the proto image (proto/Dockerfile), NOT the Elixir node
# image (stevenprice/ztlp-node, which is an OTP release with no `sh`/`ztlp`
# and fails with `ERROR: Unknown command sh`). Override per-deploy via
# LAUNCH_GATEWAY_IMAGE; default is the public proto mirror.
LAUNCH_GATEWAY_IMAGE = os.environ.get("LAUNCH_GATEWAY_IMAGE") or "stevenprice/ztlp-proto:v0.35.1"
# Per-tenant NS admin-API secret wiring. When the tenant Bootstrap needs to
# read NS records (device reconciliation), it authenticates to the NS HTTP
# admin API (/admin/records, /admin/audit on :9103) with a per-tenant
# HMAC secret. Launch generates that secret at provision time, hands it to
# the Bootstrap via secrets.env (ZTLP_NS_ADMIN_API_SECRET), and emits the
# matching NS-side env triplet (SECRET/ZONE_GLOB/CIDRS) into a per-tenant
# file under LAUNCH_NS_ADMIN_TENANTS_DIR for an operator to paste into the
# NS container env. The NS bridge name tenants must join to reach ns:9103.
LAUNCH_NS_BRIDGE = os.environ.get("LAUNCH_NS_BRIDGE") or "ztlp_default"
LAUNCH_NS_ADMIN_TENANTS_DIR = os.environ.get("LAUNCH_NS_ADMIN_TENANTS_DIR") or "/app/data/ns-admin-tenants"
# CIDR allowlist a provisioned tenant's Bootstrap presents from when it
# calls the NS admin API. Default covers the shared NS bridge (where
# tenants are attached) + loopback (host-local verification).
LAUNCH_NS_ADMIN_DEFAULT_CIDRS = os.environ.get("LAUNCH_NS_ADMIN_DEFAULT_CIDRS") or "172.18.0.0/16,127.0.0.0/8"
...
"""
    ...
    def _emit_ns_admin_tenant(self, slug: str, zone: str, secret_hex: str) -> Optional[str]:
        """Write the NS-side admin tenant triplet for a provisioned tenant.

        NS enforces per-tenant admin-API isolation from three env vars read at
        boot by ZtlpNs.AdminApi.TenantRegistry:

            ZTLP_NS_ADMIN_API_TENANT_<SLUG>_SECRET
            ZTLP_NS_ADMIN_API_TENANT_<SLUG>_ZONE_GLOB
            ZTLP_NS_ADMIN_API_TENANT_<SLUG>_CIDRS

        Launch and NS are independent compose projects, and NS holds every
        tenant's records — Launch must NOT restart it. Instead we drop a
        per-tenant dotenv fragment under LAUNCH_NS_ADMIN_TENANTS_DIR that an
        operator concatenates into the NS container env on the next NS
        recreate. Writing it is idempotent (overwritten every provision) so
        the triplet always matches the tenant's secrets.env.

        The env-var SLUG segment is the zone slug upper-cased with every
        non-alphanumeric char turned into `_` (so `acme-corp` -> `ACME_CORP`),
        matching the TenantRegistry parser which splits on the fixed
        ZTLP_NS_ADMIN_API_TENANT_ prefix and the _SECRET/_ZONE_GLOB/_CIDRS
        suffixes.
        ...
        import re
        import sys
        env_slug = re.sub(r"[^A-Za-z0-9]", "_", slug).upper()
        # zone is e.g. "acme.trs.ztlp"; the tenant should see its whole zone
        # subtree, so the glob is "*.<zone>". Exact-zone records (the bare
        # "acme.trs.ztlp" SVC, if any) are covered separately by the NS
        # operator if needed; the leading-"*." form is what TenantRegistry
        # validates and is correct for per-device records under the zone.
        zone_glob = f"*.{zone}" if zone else "*"
        cidrs = LAUNCH_NS_ADMIN_DEFAULT_CIDRS

        os.makedirs(LAUNCH_NS_ADMIN_TENANTS_DIR, exist_ok=True)
        out_path = os.path.join(LAUNCH_NS_ADMIN_TENANTS_DIR, f"{slug}.env")
        body = (
            f"# ZTLP NS admin tenant — generated by Launch for zone {zone}\n"
            f"# Concatenate into the NS container environment and recreate ns.\n"
            f"ZTLP_NS_ADMIN_API_TENANT_{env_slug}_SECRET={secret_hex}\n"
            f"ZTLP_NS_ADMIN_API_TENANT_{env_slug}_ZONE_GLOB={zone_glob}\n"
            f"ZTLP_NS_ADMIN_API_TENANT_{env_slug}_CIDRS={cidrs}\n"
        )
        ...
        try:
            ...
            pass
        print(f"_emit_ns_admin_tenant: wrote {out_path} (tenant {env_slug}, glob {zone_glob})", file=sys.stderr)
        return out_path
    ...
    def _provision_zone_dockers(self, row: sqlite3.Row, pubkey_hex: str = "") -> Optional[dict]:
        ...
        try:
            ...
            # Per-tenant NS admin-API HMAC secret. Generated ONCE (64-hex =
            # 32 raw bytes, the shape ZtlpNs.AdminApi.TenantRegistry expects)
            # and persisted in secrets.env as ZTLP_NS_ADMIN_API_SECRET so the
            # Bootstrap signs /admin/records + /admin/audit calls with it.
            # The NS side must carry the MATCHING tenant triplet (see the
            # ns-admin-tenants emission below). We read-back the existing
            # secret on re-provision so the NS triplet stays in sync and we
            # never rotate a live tenant's key.
            ns_admin_secret = ""
            if os.path.exists(secrets_env_path):
                ...
                try:
                    with open(secrets_env_path, "r", encoding="utf-8") as fh:
                        for line in fh:
                            if line.startswith("ZTLP_NS_ADMIN_API_SECRET="):
                                ns_admin_secret = line.split("=", 1)[1].strip()
                                break
                except OSError:
                    pass
                # Back-fill the admin secret for instances provisioned before
                # this field existed, so existing tenants gain admin access on
                # their next re-provision without a full teardown.
                if not ns_admin_secret:
                    ns_admin_secret = secrets.token_hex(32)
                    try:
                        with open(secrets_env_path, "a", encoding="utf-8") as fh:
                            fh.write(f"ZTLP_NS_ADMIN_API_SECRET={ns_admin_secret}\n")
                    except OSError:
                        pass
            else:
                ns_admin_secret = secrets.token_hex(32)
                with open(secrets_env_path, "w", encoding="utf-8") as fh:
                    fh.write(
                        ...
                        # Per-tenant NS admin-API HMAC secret. The Bootstrap
                        # signs admin reads against NS (/admin/records,
                        # /admin/audit on :9103) with this. NS identifies the
                        # tenant by matching this secret in its TenantRegistry
                        # and scopes the response to the tenant's zone glob.
                        f"ZTLP_NS_ADMIN_API_SECRET={ns_admin_secret}\n"
                    )
            ...
            # ── Security add-on: emit the NS-side tenant triplet ──────────
            # NS enforces per-tenant admin isolation from three env vars it
            # reads at boot:
            #   ZTLP_NS_ADMIN_API_TENANT_<SLUG>_SECRET     (== ns_admin_secret)
            #   ZTLP_NS_ADMIN_API_TENANT_<SLUG>_ZONE_GLOB  (*.{zone})
            #   ZTLP_NS_ADMIN_API_TENANT_<SLUG>_CIDRS      (bridge + loopback)
            # Launch can't restart the NS container itself (separate compose
            # project, and NS holds every tenant's records), so we WRITE the
            # triplet to a per-tenant dotenv file the operator pastes into the
            # NS env on the next NS recreate. Idempotent — overwritten each
            # provision so the secret/glob/cidrs always match secrets.env.
            try:
                self._emit_ns_admin_tenant(slug, zone, ns_admin_secret)
            except Exception as exc:  # never let this block provisioning
                print(f"_emit_ns_admin_tenant failed for {slug}: {exc}", file=sys.stderr)
            ...
            compose_yaml = (
                ...
                # Security add-on: the Bootstrap reconciler authenticates to the
                # NS gated admin HTTP API (/admin/records, /admin/audit) using
                # ZTLP_NS_ADMIN_API_SECRET (supplied via secrets.env above). It
                # needs the base URL too. NS publishes :9103 on the same host as
                # the resolver (LAUNCH_NS_SERVER host part); the bootstrap reaches
                # it through the docker bridge gateway -> host-published port.
                f"      ZTLP_NS_ADMIN_BASE_URL: \"http://{LAUNCH_NS_SERVER.split(':')[0]}:9103\"\n"
